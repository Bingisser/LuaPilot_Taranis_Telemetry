--#########################################################################################
--#   LuaPilot v2.007  Lua Telemetry Script for Taranis                                   #
--#                                                                                       #
  ...
  if (battpercent < (lastsaynbattpercent-10)) then --only say in 10 % steps
    ...
    if Time[6]> 700 then --and only say if battpercent 10 % below for more than 10sec
      ...
      if lastsaynbattpercent <= 10 then 
        playFile("/SCRIPTS/TELEMETRY/LP/SOUNDS/batcrit.wav") 
      end
    end
  ...
  ---if Resistance okay correctet Voltage else counterror
    ...
    if ((HVlipoDetected==1 and CellVolt+temp>4.45) or (CellVolt+temp>4.3 and HVlipoDetected==0)) then --not in Range
      ...
      if ResCalcError ==5 then 
        playFile("/SCRIPTS/TELEMETRY/LP/SOUNDS/errorres.wav")
        ArrayIteam=0
      end
...
--------------------------------------------------------------------------------
  local function AlarmifmaxMah()
    if BatterymahAlarm  > 0 and BatterymahAlarm < totalbatteryComsum then 
      playFile("/SCRIPTS/TELEMETRY/LP/SOUNDS/battcns.wav")
      BatterymahAlarm=0
    end
	end
 ...
 local function AlarmifOverAmp() 
    ...
    if currAvarg  > MaxAvarageAmpere  then 
        ...
        if Time[4] >=250 then --2,5s
          playFile("/SCRIPTS/TELEMETRY/LP/SOUNDS/currdrw.wav")
          Time[4] = 0
        end
      oldTime[4] = getTime()
    end
...
--lokalize optimaztion
  ...
  -- ###############################################################
    ...
    lcd.drawPixmap(1, 3, "/SCRIPTS/TELEMETRY/LP/IMAGES/battery.bmp")
...
-- ###############################################################
    ...
    lcd.drawPixmap(164, 6, "/SCRIPTS/TELEMETRY/LP/IMAGES/RSSI"..math.ceil(rxpercent*0.1)..".bmp") --Round rxpercent to the next higer 10 Percent number and search&draw pixmap
...
local FlightModeName = {}
  ...
  if data.flightmodeNr < 0 or data.flightmodeNr > 30 then
    ...
    if data.flightmodeNr~=lastflightModeNumber and SayFlightMode == 1 then
      playFile("/SCRIPTS/TELEMETRY/LP/SOUNDS/AVFM"..data.flightmodeNr.."A.wav")
...
-- ###############################################################
    ...
    if data.flightmodeNr == 6 or data.flightmodeNr == 9 or data.flightmodeNr == 28 or data.flightmodeNr == 29 then
      lcd.drawPixmap(50, 2, "/SCRIPTS/TELEMETRY/LP/IMAGES/H.bmp")  
    elseif (data.flightmodeNr >= 0 and data.flightmodeNr <= 2) or (data.flightmodeNr >= 18 and data.flightmodeNr <= 23) then
      lcd.drawPixmap(50, 2, "/SCRIPTS/TELEMETRY/LP/IMAGES/stab.bmp")
    elseif data.flightmodeNr~=-1 or data.flightmodeNr~=12 then
      lcd.drawPixmap(50, 2, "/SCRIPTS/TELEMETRY/LP/IMAGES/gps.bmp")
    end
...
-- ###############################################################
    ...
    elseif gpsFix >= 4 then
        ...
        if GPSOKAY==1 and satCount>6 then
          GPSOKAY=3
          playFile("/SCRIPTS/TELEMETRY/LP/SOUNDS/gps.wav") 
          playFile("/SCRIPTS/TELEMETRY/LP/SOUNDS/good.wav") 
        end
    ...
    elseif gpsFix == 2 then
        ...
        if GPSOKAY==3 then
          GPSOKAY=1
          playFile("/SCRIPTS/TELEMETRY/LP/SOUNDS/gps.wav") 
          playFile("/SCRIPTS/TELEMETRY/LP/SOUNDS/bad.wav")
        end
    end
...
local function run(event)
  ...
  if firsttime==0 then
    playFile("/SCRIPTS/TELEMETRY/LP/SOUNDS/welcome.wav") 
  end
  if firsttime<60 then
    lcd.drawPixmap(0, 0, "/SCRIPTS/TELEMETRY/LP/IMAGES/LuaPiloL.bmp")
    lcd.drawPixmap(106, 0, "/SCRIPTS/TELEMETRY/LP/IMAGES/LuaPiloR.bmp")
    firsttime=firsttime+1
    return 0
  end
  
  if event ==  64  then --if menu key pressed that Reset All Variables.  
    playFile("/SCRIPTS/TELEMETRY/LP/SOUNDS/reset.wav")
    killEvents(64) 
    ResetVar() 
  end
